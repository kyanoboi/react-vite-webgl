  ...
  async init(gl: WebGL2RenderingContext | null) {
    ...
    if (!this.diffuseMap) {
      this.diffuseMap = await loadTexture({
        path: new URL("./images/toy_box_diffuse.png", import.meta.url).href,
        gl,
      });
    }
    ...
    if (!this.normalMap) {
      this.normalMap = await loadTexture({
        path: new URL("./images/toy_box_normal.png", import.meta.url).href,
        gl,
      });
    }
    ...
    if (!this.heightMap) {
      this.heightMap = await loadTexture({
        path: new URL("./images/toy_box_disp.png", import.meta.url).href,
        gl,
      });
    }
