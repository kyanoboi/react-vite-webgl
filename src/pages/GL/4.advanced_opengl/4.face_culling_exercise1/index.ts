...
import shader_depthTesting from "./shader/blending_sort_glsl.ts";
...
export default class Constructor {
  ...
  async init(gl: WebGL2RenderingContext | null) {
    ...
    const windowTexture = await this.loadTexture(
      "./images/window.png",
      gl.RGBA
    );
    // 设置顶点位置
    ...
    const reverseList = this.sortByDistance(this.windows);
    for (let i = 0; i < reverseList.length; i++) {
      const model3 = mat4.create();
      mat4.translate(model3, model3, reverseList[i]);
